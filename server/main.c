        ...
        case ONGOING:
            ...
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
                if (pGame->clients[i].active && pGame->snakes[i])
                {
                    ...
                    {
                        printf("[KILL] Snake %d is dead. Killing snake.\n", i);
...
void sendGameData(Game *pGame)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        {
            printf("[WARN] snake[%d] is NULL — skipping position\n", i);
...
void handlePacket(Game *pGame)
{
    ...
    printf("Received from client %d: x=%d y=%d alive=%d\n", clientData.clientID, clientData.x, clientData.y, clientData.alive);
